...
3. Поддержка языковых модов, сохраняющих все преимущества единой системы, и в то же время снимающих все ограничения унификации с отдельных языков;
...
Первым делом были отобраны различные анализы частотности букв для интересующих языков, из которых были выведены средние значения *(получается, метаанализ)*. Всего было отобрано 25 анализов, основанных на разных выборках. Учитывались все достоверные найденные анализы.
...
Для русского языка для базовой символьной частотности были включены следующие анализы:
...
#### Результат
Начнём с английских раскладок: <code>qwerty</code> откровенно плох, если не сказать грубее. Любая существующая раскладка (в том числе из не перечисленных) в разы и разы лучше. Если смотреть на обратный результат, то всё не так однозначно, но <code>dvorak</code>, по основным пунктам, всё-таки забирает первенство.
...
На этом этапе стоит забежать несколько наперёд и отметить, что английская раскладка, на деле является общей раскладкой латиницы, которая при помощи расширенной диакритики и языковых модов символьной раскладки позволяет писать практически на всех языках, в основе которых лежит латиница или кириллица.
Таким образом, получается единая система для всех латинице- и кириллице-основанных языков. Под системой я подразумеваю всё, что вы сейчас и далее читаете – все концепты и всё из них вытекающее, будь то фонетическая преемственность или символьное единство... Стоит уже и последнем рассказать, не так ли?
...
6.1 В `config.ini` рядом с `qphyx.*` укажите путь к папке menu;
6.2 В `config.ini` рядом с `menu.*`, после первого запуска, укажите путь к папке `qphyx`;
6.3 Перезапустите `qphyx.*`. Если пути указаны верно, `qphyx` сам перезапустит `menu`, а тот, в свою очередь, после перезапуска обзаведётся пунктом `Qphyx settings` в `message menu`;
...
<dl>
  ...
  <dd>Если вы уже ознакомились с категориями в разделе <a href="https://github.com/uqqu/layout#%D0%BC%D0%BE%D0%B4%D0%B8%D1%84%D0%B8%D0%BA%D0%B0%D1%86%D0%B8%D0%B8">"Модификации"</a>, загляните в <a href="https://github.com/uqqu/layout/blob/master/external/languages.xlsx">external/languages.xlsx</a>. Там есть ответ на ваш вопрос для <b>200+</b> языков.</dd>
